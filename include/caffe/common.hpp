...
#define INSTANTIATE_CLASS(classname) \
  ...
  template class classname<int> // Kaixiang MO, 25th April, 2014
...
inline dim3 CAFFE_GET_BLOCKS2D(const int N) {
  int blocks = (N + CAFFE_CUDA_NUM_THREADS - 1) / CAFFE_CUDA_NUM_THREADS;
  return dim3((blocks + 4096 - 1) / 4096, 4096);
}

inline dim3 CAFFE_GET_BLOCKS(const int N) {
  return (N + CAFFE_CUDA_NUM_THREADS - 1) / CAFFE_CUDA_NUM_THREADS;
//  return dim3((N + 4096 - 1) / 4096, N % 4096);
}
