...
int main(int argc, char** argv) {
  if (argc < 5) {
    LOG(ERROR) << "visual_net net_proto pretrained_net_proto iterations GPU_id [CPU/GPU]";
    return 0;
  }
  ...
  for (int i = 0; i < total_iter; ++i) {
    const vector<Blob<float>*>& result =
        caffe_test_net.Forward(dummy_blob_input_vec); // from Forward to ForwardBackward! data.backpropagation saves Visualize Image.
        caffe_test_net.Backward();
    LOG(INFO)<<"iteration "<<i<<" ForwardBackward complete.";
