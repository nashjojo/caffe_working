...
Dtype DropoutLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
	...
	} else { // Test when all mask is 1
		if (propagate_down) {
			//LOG(INFO) << "Visualize: Dropout layer Backward_cpu working!";
			const Dtype* top_diff = top[0]->cpu_diff();
			Dtype* bottom_diff = (*bottom)[0]->mutable_cpu_diff();
			const int count = (*bottom)[0]->count();
			for (int i = 0; i < count; ++i) {
				bottom_diff[i] = top_diff[i]* scale_;
			}
		}
	}
...
Dtype DropoutLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
	...
	if (Caffe::phase() == Caffe::TRAIN) {
		...
		if (propagate_down) {
			// LOG(INFO) << "Visualize: Dropout layer Backward_gpu working!" << scale_;
			const Dtype* top_diff = top[0]->gpu_diff();
			Dtype* bottom_diff = (*bottom)[0]->mutable_gpu_diff();
			const int count = (*bottom)[0]->count();

			// In-place computation! must copy first!
			CUDA_CHECK(cudaMemcpy(bottom_diff, top_diff,
				count * sizeof(Dtype), cudaMemcpyDeviceToDevice));
			caffe_gpu_scal<Dtype>(count, scale_, bottom_diff);
		}
	}
