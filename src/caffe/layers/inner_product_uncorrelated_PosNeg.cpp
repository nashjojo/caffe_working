...
void InnerProductLayerUncorrelatedPosNeg<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
			vector<Blob<Dtype>*>* top) {
	CHECK_EQ(bottom.size(), 4) << "IP Layer takes 3 blobs as input, 2nd is the extra features, 3rd: positive weight, 4th: negative weight";
	...
	feature_mean_blobs[5].reset(new Blob<Dtype>(1, 1, 1, M_));
	feature_mean_blobs[6].reset(new Blob<Dtype>(1, 1, 1, 1));
};

template <typename Dtype>
void InnerProductLayerUncorrelatedPosNeg<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
		vector<Blob<Dtype>*>* top) {
...
template <typename Dtype>
Dtype InnerProductLayerUncorrelatedPosNeg<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
	...
	const Dtype* neg_weight = (*bottom)[3]->cpu_data();
	...
	*/
	...
	caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, M_, N_, num_extfeature, lambda / num_extfeature,
...
template <typename Dtype>
void InnerProductLayerUncorrelatedPosNeg<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
		vector<Blob<Dtype>*>* top) {
...
template <typename Dtype>
Dtype InnerProductLayerUncorrelatedPosNeg<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
		const bool propagate_down,
		vector<Blob<Dtype>*>* bottom) {
	...
	const Dtype* neg_weight = (*bottom)[3]->gpu_data();
	...
	// 		std::cout << top[0]->cpu_data()[i*N_+j] << ",\t";
	// 	}
	// 	std::cout << std::endl;
	// }
	...
	// feature_mean_blobs[5] = weight
  CUDA_CHECK(cudaMemcpy(feature_mean_blobs[5]->mutable_gpu_data(),
  	weight, sizeof(Dtype) * M_,
  	cudaMemcpyDeviceToDevice));
  // feature_mean_blobs[5] += neg_weight
	caffe_gpu_axpy(M_, (Dtype)1., neg_weight, feature_mean_blobs[5]->mutable_gpu_data());
	// feature_mean_blobs[6] = sum(feature_mean_blobs[5])
	caffe_gpu_gemv<Dtype>(CblasTrans, M_, 1, (Dtype)1., feature_mean_blobs[5]->gpu_data(),
		reinterpret_cast<const Dtype*>(bias_multiplier_->gpu_data()), (Dtype)0., 
		feature_mean_blobs[6]->mutable_gpu_data());

	const Dtype* ins_weight = feature_mean_blobs[5]->cpu_data();
	const Dtype ins_weight_sum = feature_mean_blobs[6]->cpu_data()[0]; // must be cpu data!

	// visualize instance weight
	// std::cout << "ins_weight" << std::endl;
	// for (int i = 0; i < M_; ++i ) {
	// 	std::cout << feature_mean_blobs[5]->cpu_data()[i] << ",\t";
	// }
	// std::cout << std::endl;
	// std::cout << "ins_weight_sum:" << ins_weight_sum << std::endl;

	// modify top_diff so as to penalize large correlation

	// mean for output: after careful consideration
	caffe_gpu_gemv<Dtype>(CblasTrans, M_, N_, (Dtype)1. / ins_weight_sum, top_data,
			reinterpret_cast<const Dtype*>(feature_mean_blobs[5]->gpu_data()), (Dtype)0., 
			feature_mean_blobs[0]->mutable_gpu_data());

	// mean for extfeature
	caffe_gpu_gemv<Dtype>(CblasTrans, M_, num_extfeature, (Dtype)1. / ins_weight_sum, extfeature_data,
			reinterpret_cast<const Dtype*>(feature_mean_blobs[5]->gpu_data()), (Dtype)0., 
	...
	// for (int j = 0; j < min(N_, num_col); ++j) {
	// 	std::cout << feature_mean_blobs[0]->cpu_data()[j] << ",\t";
	// }
	// std::cout << std::endl;
	...
	// normalize to mean 0
	for (int j = 0; j < M_; ++j) {
		caffe_gpu_axpy<Dtype>(N_, (Dtype)-1., feature_mean_blobs[0]->gpu_data(), feature_mean_blobs[2]->mutable_gpu_data() + j*N_ );
	}
	...
	// normalize to mean 0
	for (int j = 0; j < M_; ++j) {
		caffe_gpu_axpy<Dtype>(num_extfeature, (Dtype)-1., feature_mean_blobs[1]->gpu_data(), feature_mean_blobs[3]->mutable_gpu_data() + j*num_extfeature);
	}
	...
	// for (int i = 0; i < M_; ++i){
	// 	for (int j = 0; j < min(N_, num_col); ++j) {
	// 		std::cout << feature_mean_blobs[2]->cpu_data()[i*N_+j] << ",\t";
	// 	}
	// 	std::cout << std::endl;
	// }
	// std::cout << "extfeature_data - mean" << std::endl;
	// for (int i = 0; i < M_; ++i){
	// 	for (int j = 0; j < min(num_extfeature, num_col); ++j) {
	// 		std::cout << feature_mean_blobs[3]->cpu_data()[i*num_extfeature+j] << ",\t";
	// 	}
	// 	std::cout << std::endl;
	// }

	// (output feature - mean) .* weight
	for (int i = 0; i < M_; ++i) {
		caffe_gpu_scal(num_extfeature, feature_mean_blobs[5]->cpu_data()[i], feature_mean_blobs[3]->mutable_gpu_data() + i*num_extfeature);
	}

	// std::cout << "(extfeature_data - mean).* ins_weight" << std::endl;
	// for (int i = 0; i < M_; ++i){
	// 	for (int j = 0; j < min(num_extfeature, num_col); ++j) {
	// 		std::cout << feature_mean_blobs[3]->cpu_data()[i*num_extfeature+j] << ",\t";
	// 	}
	...
	// correlation,     [Need examination carefully]
	// [(extfeature-mean) .* weight] *(output-mean) in a single matrix multiplication
	caffe_gpu_gemm<Dtype>(CblasTrans, CblasNoTrans, num_extfeature, N_, M_, (Dtype)1. / ins_weight_sum,
	...
	// for (int i = 0; i < num_extfeature; ++i){
	// 	for (int j = 0; j < min(N_, num_col); ++j) {
	// 		std::cout << feature_mean_blobs[4]->cpu_data()[i*N_+j] << ",\t";
	// 	}
	// 	std::cout << std::endl;
	// }
	...
	caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, M_, N_, num_extfeature, lambda / (num_extfeature * ins_weight_sum),
...
INSTANTIATE_CLASS(InnerProductLayerUncorrelatedPosNeg);
