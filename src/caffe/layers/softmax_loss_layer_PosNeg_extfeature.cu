...
void SoftmaxWithLossLayerPosNegExtFeature<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
			vector<Blob<Dtype>*>* top) {
	CHECK_EQ(bottom.size(), 6) << "SoftmaxLoss Layer takes 6 blobs as input, 1:data 2:label 3:weight 4:id 5:negative-weight 6:extra-feature";
...
template <typename Dtype>
void SoftmaxWithLossLayerPosNegExtFeature<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
		vector<Blob<Dtype>*>* top) {
...
template <typename Dtype>
void SoftmaxWithLossLayerPosNegExtFeature<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
		vector<Blob<Dtype>*>* top) {
	// The forward pass computes the softmax prob values.
	softmax_bottom_vec_[0] = bottom[0];
	softmax_layer_->Forward(softmax_bottom_vec_, &softmax_top_vec_);
...
template <typename Dtype>
Dtype SoftmaxWithLossLayerPosNegExtFeature<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
	...
	// No need to specify
	const int num_extfeature = (*bottom)[5]->channels();
	...
	for (int j = 0; j < num; j++) {
		instance_weight.push_back(weight[j] + neg_weight[j]);
	...
	for (int j = 0; j < dim; j++) {
		...
		Dtype co = Correlation( prob[j], p_mean[j], feature[k], f_mean[k], instance_weight );// the weight of each instance
	...
	for (int i = 0; i < num; ++i) {
		...
		for (int j = 0; j < dim; j++) {
			for (int k = 0; k < num_extfeature; k++) {
				bottom_diff[i * dim + j] += (weight[i]+neg_weight[i])* lambda*covar[j][k]*(feature[k][i]-f_mean[k]) / num_extfeature;
			}
		}
	}
...
template <typename Dtype>
Dtype SoftmaxWithLossLayerPosNegExtFeature<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
...
template <typename Dtype>
Dtype SoftmaxWithLossLayerPosNegExtFeature<Dtype>::Correlation(const vector<Dtype>& feature_1, Dtype mean_1, 
...
INSTANTIATE_CLASS(SoftmaxWithLossLayerPosNegExtFeature);
