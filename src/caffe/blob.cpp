...
template <typename Dtype>
void Blob<Dtype>::Clear(bool clear_diff){
  switch (Caffe::mode()) {
  case Caffe::GPU:
    if (clear_diff){ 
      CUDA_CHECK(cudaMemset(diff_->mutable_gpu_data(), 0, 
            sizeof(Dtype) * count_));
    }
    CUDA_CHECK(cudaMemset(data_->mutable_gpu_data(), 0, 
            sizeof(Dtype) * count_));
    break;

  case Caffe::CPU:
    if (clear_diff){
      memset(diff_->mutable_cpu_data(), 0,
          sizeof(Dtype) * count_);
    }
    memset(data_->mutable_cpu_data(), 0,
        sizeof(Dtype) * count_);
    break;
