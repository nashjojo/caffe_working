...
const vector<Blob<Dtype>*>& Net<Dtype>::ForwardPrefilled() {
  for (int i = 0; i < layers_.size(); ++i) {
	//LOG(INFO)<<i<<" "<<top_vecs_[i].size();
    // LOG(ERROR) << "Forwarding " << layer_names_[i];
    layers_[i]->Forward(bottom_vecs_[i], &top_vecs_[i]);
	//LOG(INFO)<<"finished forwarding layer "<<i;
  }
...
void Net<Dtype>::Update() {
  ...
  // We renormalize each filter according to Zeiler 2013'
  for (int i = 0; i < layers_.size(); ++i){
    if (layer_types_[i] == "conv"){
      boost::dynamic_pointer_cast<ConvolutionLayer<Dtype> >(layers_[i])
        ->RenormalizeFilter();
    }
  }
}
