...
void Solver<Dtype>::Solve(const char* resume_file) {
  ...
  Dtype hist_loss = Dtype(0.);
  while (iter_++ < param_.max_iter()) {
    Dtype loss = DoUpdate();  // major entry point for various solver
	
	if (hist_loss == Dtype(0.)) {
	  hist_loss = loss;
	} else {
	  hist_loss = (hist_loss * (param_.snapshot() - 1) + loss) / (1.0 * param_.snapshot()) ;
	}

    if (param_.display() && iter_ % param_.display() == 0) {
      LOG(INFO) << "Iteration " << iter_ << ", loss = " << loss << ", hist_loss = " << hist_loss;
    }
...
Dtype SGDSolver<Dtype>::DoUpdate() {
  ...
  if (this->iter_ % updateInterval == 0){
    ...
    ComputeUpdateValue(); // Compute the value to history, and then copy them to the blob's diff.
    this->net_->Update();
    for (int param_id = 0; param_id < net_params.size(); ++param_id) {
...
Dtype NesterovSolver<Dtype>::DoUpdate() {
  ...
  // Jump the momentum first, move from x to (x+v_t), where we need to compute gradient
  int updateInterval = this->GetUpdateInterval();
  if (updateInterval == 1 || this->iter_ % updateInterval == 1){
    for (int param_id = 0; param_id < net_params.size(); ++param_id) {
      ...
      // move network parameter from x to (x + v_t), then we will compute gradient at new point (x+v_t)
      caffe_gpu_copy(this->history_[param_id]->count(),
        this->history_[param_id]->gpu_data(),
        net_params[param_id]->mutable_gpu_diff()
      );
    }
  ...
  Dtype loss = this->net_->ForwardBackward(bottom_vec);
  for (int param_id = 0; param_id < net_params.size(); ++param_id) {
    // add all gradient to "accumulate_"
    caffe_gpu_axpy(this->accumulate_[param_id]->count(), Dtype(1),
         net_params[param_id]->gpu_diff(),
         this->accumulate_[param_id]->mutable_gpu_data()
      );
